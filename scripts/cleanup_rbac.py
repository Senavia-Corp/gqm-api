...
def main() -> None:
    with get_session() as s:
        ...
        missing = MANAGED_ROLES - roles.keys()
        if missing:
            sys.exit(f"⛔ faltan roles gestionados {missing} — corre antes seed_rbac.py")
        ...
        for m in s.exec(select(Member)).all():
            ...
            target = None
            if email in FULL_ADMIN_EMAILS:
                target = full_admin
            elif email in GQM_MEMBER_EMAILS:
                target = gqm_member
            if target and m.ID_Role != target.ID_Role:
                print(f"  rol → {target.Name}: {m.ID_Member} {email}")
                m.ID_Role = target.ID_Role
                s.add(m)
            elif not target and m.ID_Role is None:
                print(f"  ⚠️ sin mapeo confirmado (queda sin rol): {m.ID_Member} {email}")
        s.commit()
