...
def podio_webhook(app_type):
    ...
    # Mapeo genérico de app_type → router, mapper, modelo, id_field
    APP_ROUTER_MAP = {
    ...
    try:
        ...
        with get_session() as session:
            ...
            # -----------------------------
            # Generar ID interno si no viene de Podio
            # -----------------------------
            if not item_data.get(id_field, None):
                prefix = PREFIX_MAP.get(app_type, "GEN")
                new_id = generate_custom_id(session, Model, id_field, prefix)
                item_data[id_field] = new_id
                print(
                    f"🆔 Generado nuevo ID interno para {Model.__name__}: {new_id}")
            ...
            # -----------------------------
                ...
                obj = session.exec(select(Model).where(
                    Model.podio_item_id == item_id)).first()
                if obj:
                    session.delete(obj)
                    session.commit()
