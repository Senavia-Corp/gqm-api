...
def recalculate_document_from_db(session, qbo_id):
    ...
    # Recalcular balance sumando pagos vinculados
    total_paid = sum(
        link.transaction.Amount
        for link in doc.transactions_links
    )

    doc.Balance_Amount = doc.Total_Amount - total_paid
...
def _handle_invoice(session, entity, realm_id, dry_run):
    ...
    if doc_obj:
        # Items: Procesamiento en lote (bulk) si es posible
