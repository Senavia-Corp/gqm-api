...
def sync_job_financials(realm_id, job_code, start, limit, dry_run):
    ...
    try:
        ...
        print(
            f"✅ Invoices procesadas: {invoice_result.get('processed_count', 0)}")
    except Exception as e:
        print(f"❌ ERROR en fase de Invoices: {str(e)}")
        invoice_result = {"error": str(e), "processed_count": 0}
    ...
    try:
        bill_result = sync_qbo_bills_and_payments_by_job(
            realm_id, job_code, start, limit, dry_run)
        print(f"✅ Bills procesados: {bill_result.get('processed_count', 0)}")
    except Exception as e:
        print(f"❌ ERROR en fase de Bills: {str(e)}")
        bill_result = {"error": str(e), "processed_count": 0}

    # --- FASE 3: RESUMEN FINAL ---
    total = invoice_result.get('processed_count', 0) + \
        bill_result.get('processed_count', 0)
    print("-" * 50)
