...
def sync_qbo_bills_and_payments_by_job(
    ...
    for bill in bills:
        ...
        for txn in linked_txns:
            if txn.get("TxnType") in ("BillPaymentCheck", "BillPaymentCreditCard"):
    ...
    with get_session() as session:
        ...
        for bill in bills:
            ...
            # -----------------------------
            doc_obj, created = upsert_financial_document(
                session=session,
                data=mapped_doc,
            ...
            for txn in linked_txns:
                ...
                if txn.get("TxnType") not in ("BillPaymentCheck", "BillPaymentCreditCard"):
                    continue
                ...
                trans_obj, created = upsert_financial_transaction(
                    session=session,
                    data=mapped_trans,
